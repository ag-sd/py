...
def download_image(img_link):
    ...
    if img_link is not None:  # ensure that we got a valid full-size jpeg background image.
        ...
        file_name = f"{datetime.date.today()}-{parse.parse_qs(parse.urlparse(img_link).query)['id'][0]}"
        file_path = f"/home/sheldon/.sd-custom/art/wallpaper/bing/{file_name}"
        if os.path.exists(file_path):
            print("File already exists. Return existing file")
            return file_path
        img_file = open(file_path, 'wb')  # create file named with last part of the url.
